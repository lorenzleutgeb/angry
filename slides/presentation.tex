	...
	\begin{center}
		\begin{figure}
			\begin{tikzpicture}
				...
				\begin{uncoverenv}<2->
					\node[anchor=south, alerted text.fg] (plan) at (5.5,5.4) {\Large Planning};
					\draw[alerted text.fg,ultra thick,rounded corners] (5.0,1.65) rectangle (5.95,2.4);
					\draw [-stealth, alerted text.fg,ultra thick,rounded corners] (plan) -- (5.45,2.5);
					\node[anchor=west, alerted text.fg] (tactic) at (7.1, 6.0) {Tactic};
					\node[anchor=west, alerted text.fg] (strategy) at (7.1, 5.3) {Strategy};
					\draw [-stealth, alerted text.fg,ultra thick,rounded corners] (strategy) -- (6.5,5.6);
					\draw [-stealth, alerted text.fg,ultra thick,rounded corners] (tactic) -- (6.5,5.8);
				\end{uncoverenv}
				\begin{uncoverenv}<3->
...
\begin{frame}{Pros and Cons of our Modifications}
	\begin{itemize}[<+->]
		...
		\item[$+$] Ordinary ASP programs are easier to read and understand.
		\item[$+$] Alpha uses lazy-grounding (impact not well analyzed).
